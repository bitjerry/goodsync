...
int aes_encrypt_master_key(unsigned char *master_key, int master_key_len, unsigned char *out, int *out_len) {
    unsigned char salt[] = "Jf5tuaDP";
    unsigned char key[] = "CD7qLuyoS9";
    ...
    if (!EVP_DigestFinal_ex(ctx_md, d2, NULL)) {
        return MASTER_KEY_DIGEST_FINAL_ERROR;
    }

    EVP_MD_CTX_destroy(ctx_md);

    if (!EVP_EncryptUpdate(ctx, key_out + offset, &key_out_len, d2, 8)) {
        EVP_CIPHER_CTX_free(ctx);
        return MASTER_KEY_ENCRYPT_DIGEST_UPDATE_ERROR;
    }

    unsigned char d3[4] = {master_key_len, 0x00};
    if (!EVP_EncryptUpdate(ctx, key_out + offset + key_out_len, &key_out_len, d3, 4)) {
        EVP_CIPHER_CTX_free(ctx);
    ...
    unsigned char d1[] = {0x01, 0x00, 0x0a, 0x00, 0x40, 0x00, 0x4a, 0x66, 0x35, 0x74, 0x75, 0x61, 0x44, 0x50};
    for (int i = 0; i < 14; ++i) {
        key_out[i] = d1[i];
    }
